...
Result<BodyStatment_Result<SuperConditionalStatment>> convertElseStatment(TokenIterator& iterator, MetaData& metaData, FuncDeclaration& funcInfo, CurrentContext& context)
{
    ...
    return BodyStatment_Result<SuperConditionalStatment>
        (
            make_shared<ElseStatment>(ElseStatment(ifBodyResult.value()))
        );
}
...
{
    if (!iterator.nextToken())
        return ErrorInfo("enxpected end while parsing 'else if' statment", iterator.currentLine);
    ...
    auto body = BodyStatment_Result<SuperConditionalStatment>
        (
            make_shared<ElseIfStatment>(ElseIfStatment(conditionResult.value().expression, ifBodyResult.value()))
        );
...
Result<BodyStatment_Result<SuperConditionalStatment>> convertIfStatment(TokenIterator& iterator, MetaData& metaData, FuncDeclaration& funcInfo, CurrentContext& context)
{
    ...
    auto body = BodyStatment_Result<SuperConditionalStatment>
        (
            make_shared<IfStatment>(IfStatment(conditionResult.value().expression, ifBodyResult.value()))
        );
